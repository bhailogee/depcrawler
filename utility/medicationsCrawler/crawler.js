/**
 * Created by mwaseem on 4/10/2017.
 */
...
function crawlConditions(){
    ...
    var conditionUrls=[];
    for (var i = 65; i <= 90; i++) {
        var cchar = String.fromCharCode(i).toLowerCase();
        conditionUrls.push('https://www.drugs.com/condition/' + cchar + '.html');
    }
    c.queue(conditionUrls.shift());
}
...
var c = new Crawler({
    ...
    callback : function (error, res, done) {
        if(error){
            ...
            var directory="condition/"+path.split('/')[path.split('/').length-1].replace('.html','');

            if (!fs.existsSync(directory)) {
                fs.mkdirSync(directory);
            }
            //save it in db conditions
                $('.column-list-2 li a').map(function() {
                    return {href: this.attributes.href.value, text: this.text};
                });
